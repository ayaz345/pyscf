...
with ao2mo.load(hesstmp, 'hessints1') as eri:
    print(
        f'(d/dR i d/dR j| kl) have shape {str(eri.shape)} due to the 4-fold permutation symmetry i >= j, k >= l'
    )
...
print(
    f"(d/dR d/dR i j| kl) have shape {str(feri['hessints2'].shape)} due to the 2-fold permutation symmetry k >= l"
)
feri.close()
...
with ao2mo.load(ao2mo.kernel(mol, orb, hesstmp.name, intor='cint2e_ip1ip2_sph',
                             aosym='s1')) as eri:
    print(
        f'(d/dR i j|d/dR k l) have shape {str(eri.shape)} because there is no permutation symmetry'
    )
