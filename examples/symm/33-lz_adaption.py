...
# See L146 of pyscf/symm/basis.py
    for ir in Ex_irrep_ids:
        ...
        Ey = mol.irrep_id.index(ir+1) if is_gerade else mol.irrep_id.index(ir-1)
        lz = (ir // 10) * 2 if ir % 10 in (0, 5) else (ir // 10) * 2 + 1
        ...
        lz_plus = (
            numpy.sqrt(0.5) * (mol.symm_orb[Ex] + mol.symm_orb[Ey] * 1j)
            if lz % 2 == 0
            else -numpy.sqrt(0.5) * (mol.symm_orb[Ex] + mol.symm_orb[Ey] * 1j)
        )
